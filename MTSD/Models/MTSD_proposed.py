...
names_subjects = list(names_subjects)
...
pre_name = ['/correlation_matrix/raw_correlation_matix_']
threshold_1 = 0.3

for j, tyope_name in enumerate(pre_name):
    for i, sub in enumerate(names_subjects):
        correlation_matrix = np.load(path + tyope_name + sub + '.npy')
        color, ele_row, ele_col, trials = correlation_matrix.shape
        norm_correlation_matirx = np.zeros(shape=(color, ele_row, ele_col, trials))
        temp = np.zeros(shape=(color, ele_row, ele_col, trials))
        threshold = np.zeros(shape=(color, ele_row, ele_col, trials))

        for real_trial in range(trials):
            scaler = MinMaxScaler()
            data = correlation_matrix[0, :, :, real_trial]
            tran_data = np.transpose(data)
            scaler.fit(tran_data)
            data = scaler.transform(tran_data)
            tran_data = np.transpose(data)

            tran_data[tran_data <= 0.3] = 0
            tran_data[tran_data > 0.3] = 1
            norm_correlation_matirx[0, :, :, real_trial] = tran_data
        ...
        fea_row, fea_col, fea_trial = imf1.shape

        if(fea_trial == trials):
            pass
        else:
            print('trials not equal')
            copula_excp = np.load(path + 'correlation_matrix/raw_correlation_matix_' + sub + '_excep.npy')
            left_trials = list(set(list(np.arange(fea_trial))) - set(copula_excp))
            imf1 = imf1[:, :, left_trials]
            imf2 = imf2[:, :, left_trials]
            imf3 = imf3[:, :, left_trials]
        ...
        copula_raw = np.transpose(norm_correlation_matirx, (3, 0, 1, 2))
        ...
        train_copula = copula_raw[train_index, :, :, :]
        test_copula = copula_raw[test_index, :, :, :]
        train_label = label[train_index]
        test_label = label[test_index]
        trials_t, color_t, height_t, width_t = train_data.shape

        training_data = Dataset.My_minxed_Dataset(list_IDs=list(np.arange(trials_t)),
                                                  labels=train_label,
                                                  train_data=train_data,
                                                  mask_data=train_copula,
                                                  totensor=True)
        #   hyperparameters
        b_s = 16
        b_s_t = 128
        ...
        testing_data = Dataset.My_minxed_Dataset(list_IDs=list(np.arange(t_trials)),
                                                 labels=test_label,
                                                 train_data=test_data,
                                                 mask_data=test_copula,
                                                 totensor=True)
        ...
        for epoch in range(e_p):
            ...
            for step, sample_batch in enumerate(training_loader):
                ...
                b_x = b_x.to(device)
                b_y = b_y.to(device)
                b_x_copula = sample_batch['batch_mask']
                b_x_copula = b_x_copula.to(device)

                output = model(b_x.float(), b_x_copula.float())
            ...
            with torch.no_grad():
                ...
                for step_t, sample_batch_t in enumerate(testing_loader):
                    ...
                    b_x_copula_t = sample_batch_t['batch_mask']
                    b_x_copula_t = b_x_copula_t.to(device)

                    test_output = model(b_x_t.float(), b_x_copula_t.float())
            ...
            if (epoch + 1 == 50):
                ...
                torch.save(model.state_dict(), path + '/model_parameters/'
                           + 'MTSD_' + str(b_s) + '_' + str(lr) + str(window_num) + '_'
                           + sub + '.pth')
            else:
                pass
        ...
        np.save(
            path + '/model_parameters/MTSD_' + str(1) + str(threshold_1) + '_' + str(b_s) + '_' + str(lr) + str(window_num) + '_' + sub + '_train_acc.npy',
            np.array(ave_train_acc))
        np.save(
            path + '/model_parameters/MTSD_' + str(1) + str(threshold_1) + '_' + str(b_s) + '_' + str(lr) + str(window_num) + '_' + sub + '_train_loss.npy',
            np.array(ave_train_loss))
        np.save(
            path + '/model_parameters/MTSD_' + str(1) + str(threshold_1) + '_' + str(b_s) + '_' + str(lr) + str(window_num) + '_' + sub + '_test_acc.npy',
            np.array(ave_test_acc))
