...
'''channel selection'''
path = ''
names_subjects = list(np.load('/names_subjects.npy'))
...
for i, sub in enumerate(names_subjects):
    ...
    for arch_kernel in np.arange(35, kernel_set, 10):
        ...
        kernel_t_acc.append(ave_train_acc)
        kernel_t_loss.append(ave_train_loss)
        kernel_s_acc.append(ave_test_acc)
    if(os.path.exists(path + '/model_result/')):
        pass
    else:
        os.makedirs(path + '/model_result/')

    kernel_t_acc = np.array(kernel_t_acc)
    kernel_t_loss = np.array(kernel_t_loss)
    kernel_s_acc = np.array(kernel_s_acc)
    np.save(path + '/model_result/' + 'ECNN_' + str(window_num) + '_' + sub + '_train_acc.npy', kernel_t_acc)
    np.save(path + '/model_result/' + 'ECNN_' + str(window_num) + '_' + sub + '_train_loss.npy', kernel_t_loss)
    np.save(path + '/model_result/' + 'ECNN_' + str(window_num) + '_' + sub + '_test_acc.npy', kernel_s_acc)
