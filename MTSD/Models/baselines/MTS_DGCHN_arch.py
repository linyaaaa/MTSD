...
'''following is correct: adjust it to match our dataset'''
# directly read a_hat

    def __init__(self, GCN_in, GCN_out, a_hat):
    ...
    def forward(self, X):
        # print('type {} and {}'.format(self.A_hat.dtype, X.dtype))
        lfs = torch.einsum("ij,jklm->kilm", [self.A_hat, X.permute(1, 0, 2, 3)])
        # print('lfs size ', lfs.size())
...
class TCNmodel2(nn.Module):
    ...
    def __init__(self, input_size, num_T, num_S,
        ...
        '''following is correct: adjust it to match our dataset'''
        self.Sception1 = nn.Sequential(
            nn.Conv2d(num_T, num_S, kernel_size=(108, 1), stride=1, padding=0),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=(1, 2), stride=(1, 2)))
        self.Sception2 = nn.Sequential(
            nn.Conv2d(num_T, num_S, kernel_size=(54, 1), stride=(54, 1), padding=0),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=(1, 2), stride=(1, 2)))
        self.conv = nn.Sequential(
            nn.Conv2d(num_S, num_S, kernel_size=(3, 1), stride=(1, 1), padding=0),
    ...
    def forward(self,X ):
        ...
        out_final = z
        z = self.Sception2(out)
        # print('z next size ', z.size())
        out_final = torch.cat((out_final, z), dim=2)
        out_final = self.conv(out_final)
        # print('out final size ', out_final.size())
...
class GCNmodel3(nn.Module):
    ...
    def __init__(self,  input_size, num_S,GCN_hiden1,GCN_hiden2,GCN_hiden3, 
        ...
        self.Sception1 = nn.Sequential(
            # nn.Conv2d(input_size[0], num_S, kernel_size=(30, 1), stride=1, padding=0),
          '''following is correct: adjust it to match our dataset'''
            nn.Conv2d(input_size[0], num_S, kernel_size=(108, 1), stride=1, padding=0),
    ...
    def forward(self,X ):
        ...
        # print('X size {} and out size {}'.format(X.size(), out.size()))
...
class MTS_DGCHN(nn.Module):
    ...
    def get_size(self, input_size):
        ...
        # print('outt size {} and outs size {}'.format(out_t.size(), out_s.size()))
        out = torch.cat((out_t, out_s), dim=1)
        out = self.conv(out)
        out = self.BN(out)
        out = out.view(out.size()[0], -1)
