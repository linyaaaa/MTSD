...
for sub_index, sub in enumerate(names_subjects):
    ...
    model = Model_Architecture.ESTCNN_entry(num_classes=classes,
                                            input_width=width_t,
    ...
    for epoch in range(e_p):
        ...
        if (epoch + 1 == 50):
            ...
            torch.save(model.state_dict(), path + '/model_parameters/'
                       + 'baseline_' + str(b_s) + '_' + str(lr) + str(window_num) + '_'
                       + sub + '.pth')
        else:
            pass
    ...
    np.save(path + '/model_result/' + 'ESTCNN_' + str(b_s) + '_' + str(lr) + str(window_num) + '_' + sub + '_train_acc.npy', np.array(ave_train_acc))
    np.save(path + '/model_result/' + 'ESTCNN_' + str(b_s) + '_' + str(lr) + str(window_num) + '_' + sub + '_train_loss.npy', np.array(ave_train_loss))
    np.save(path + '/model_result/' + 'ESTCNN_' + str(b_s) + '_' + str(lr) + str(window_num) + '_' + sub + '_test_acc.npy', np.array(ave_test_acc))
