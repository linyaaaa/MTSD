...
for sub_index, sub in enumerate(names_subjects):
    ...
    for epoch in range(e_p):
        ...
        if (epoch + 1 == 50):
            ...
            torch.save(model.state_dict(), path + '/model_parameters/'
                       + 'ECNN_Base_PCM_' + str(b_s) + '_' + str(lr) + str(window_num) + '_'
                       + sub + '.pth')
        else:
            pass
    ...
    np.save(path + '/model_parameters/ECNN_Base_PCM_' + str(b_s) + '_' + str(
        lr) + str(window_num) + '_' + sub + '_train_acc.npy', np.array(ave_train_acc))
    np.save(path + '/model_parameters/ECNN_Base_PCM_' + str(b_s) + '_' + str(
        lr) + str(window_num) + '_' + sub + '_train_loss.npy', np.array(ave_train_loss))
    np.save(path + '/model_parameters/ECNN_Base_PCM_' + str(b_s) + '_' + str(
        lr) + str(window_num) + '_' + sub + '_test_acc.npy', np.array(ave_test_acc))
